...
class TestPerformance:
    ...
    def _clear_registry(self):
        try:
            response = requests.get(f"{self.url}/transactions", timeout=self.timeout)
            if response.status_code == 200:
                for t in response.json():
                    requests.delete(f"{self.url}/transactions/{t['transaction_id']}", timeout=self.timeout)
        except Exception:
            pass
    ...
    def test_create_transaction_performance(self):
        for _ in range(self.iteration_count):
            response = requests.post(f"{self.url}/transactions", json=self.transaction_data, timeout=self.timeout)
    ...
    def test_get_summary_performance(self):
        ...
        for _ in range(self.iteration_count):
            response = requests.get(f"{self.url}/summary", timeout=self.timeout)
    ...
    def test_create_and_delete_cycle(self):
        for _ in range(50): # smaller loop as it involves more operations
            ...
            delete_resp = requests.delete(f"{self.url}/transactions/{t_id}", timeout=self.timeout)
